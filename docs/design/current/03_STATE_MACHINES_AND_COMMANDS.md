...
- Assignment-change LINE `引き受ける` first tap records an explicit `acceptance_intent` on the checking attempt; ordinary `確認してみる` does not. The canonical command rejects LINE acceptance of an assignment change until the attempt is checking with that explicit intent, including calls through older compatibility actions. Only explicit acceptance intent receives the final-confirmation reminder and wording in Daily Brief. Both paths retain the original assignee until final acceptance. A reminder never accepts on behalf of the recipient.
- The recipient can reopen `お願いの返事` after leaving LINE and use the current revision's `確定（引受）` action. The reader must search the unresolved request set before selecting a card; newer sent or historical requests must not hide the recipient's unfinished final confirmation. The requester sees intent without assignment success. Follow-ups have distinct idempotency keys at 10 and 60 minutes after the first tap; each requires the still-active attempt and an unexpired reply deadline. The morning/evening Daily Brief keeps the unresolved request visible without turning it into repeated standalone pushes.
- A prior ordinary `確認してみる` may later advance to explicit LINE acceptance intent through a new revision-bound first tap. This must not recast the earlier schedule check as acceptance or skip final confirmation.
- PWA Today and the Request list use the same assignment-change first tier (`引き受ける / 難しい / 相談する`) and show scope, target, deadline and dependent same-day effect before a separate final operation. The local confirmation is bound to the observed Attempt ID/revision/terms revision; a refreshed snapshot requires reviewing again. Daily Brief keeps those fields and the intent flag for Today. The requester sees status/waiting, never recipient acceptance buttons.
