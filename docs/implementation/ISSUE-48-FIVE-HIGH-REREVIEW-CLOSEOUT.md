...
| Q59 | 一括完了は即確定し、直後に例外修正/undo。 | `routine_reconciliation_operations/snapshots`; `server_tx_reconcile_routine_session_v2`; `server_tx_undo_routine_reconciliation`; `20260906000004_q59_bulk_partner_actual_correction.sql`; `CheckinPage.tsx` immediate `例外を修正` / `元に戻す`. Bulk `相手が対応` correction reuses `private.fn_command_correct_task_actual_v1`, preserving the former self performer as removed history and making the partner the sole active canonical performer. Undo remains exact-receipt scoped and revision/status CAS guarded. | `76_issue48_q59_q64_literal_regression.sql`; `77_issue48_q59_bulk_partner_correction.sql`; Checkin Web tests | パパが「全部やった」後、「ゴミ出しだけ実際はママだった」と気づき、`例外を修正 → 相手が対応`。self actualは履歴として残してactiveから外れ、ママがcanonical actual performerになる。retryは同一結果、別householdは拒否、訂正後の古いbulk undoはstaleで止まり、Historyが読むactive performerもママを指す。 | PASS |
...
## Q59 bulk correction / concurrency / undo boundary

Bulk completion creates a durable operation receipt and snapshots only eligible rows. The hardening trigger removes rows whose before/after state is identical. Undo first locks and validates every retained task against its exact post-operation revision and status; if any task changed later it raises `RECONCILIATION_UNDO_STALE` before restoring anything.

The independent re-review found one cross-flow gap at the intersection of Q59 and Q64: after `all_done`, a completed task corrected to `partner_handled` previously changed only `routine_item_reconciliation_outcomes`, leaving the actual performer as self. `20260906000004_q59_bulk_partner_actual_correction.sql` now intercepts that exact bulk-correction path and calls the existing canonical actual-correction command. The old self participant is retained with `removed_at`, the partner becomes the only active `task_actual_participants` performer, `task_instances.actual_completed_by_id` is aligned, the reconciliation outcome is `partner_handled`, and task revision/audit history advance together.

`77_issue48_q59_bulk_partner_correction.sql` directly executes the user scenario `全部やった → 例外を修正 → 相手が対応` and asserts canonical performer correction, old-performer history retention, one active partner performer, history/read-model durable inputs, exact retry idempotency, cross-household rejection without partial mutation, and stale rejection of the old bulk undo after the later correction.
