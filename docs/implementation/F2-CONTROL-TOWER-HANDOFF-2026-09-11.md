...
| Accepted pickup changed only pickup task | role-dependent same-day task reconciliation | migration applied |
| Existing already-accepted pickup request predated dependency fix | one-time canonical backfill via the same reconcile function | completed; 10 dependent tasks updated, unresolved=0 |

## 4. Production runtime snapshot before this docs PR

Pre-documentation main:

`ad5e99d352230a94b1498c1262b32e6956f1a121`
...
Production safety readback at handoff preparation:

- active LINE links: 2
- notification outbox pending/retry/queued: 0
- Google write pending/retry/queued: 0
- latest real pickup assignment request: accepted
- today pickup: mama test member
- same-day `pickup_assignee` dependents: reconciled to mama test member

The documentation merge will advance main again. Therefore **do not freeze the pre-documentation SHA above as final F2 HEAD**. Fresh-read main after the docs PR merges.
...
6. Freeze **one** exact HEAD for the next F2 run.
7. Reclassify any old physical evidence from prior HEADs as stale/history where strict exact-head binding requires it.
8. Hand the frozen HEAD and the new-owner document to the new F2 owner.
