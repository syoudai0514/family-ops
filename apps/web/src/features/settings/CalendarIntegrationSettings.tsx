...
function oauthCallbackNotice() {
  ...
  if (params.get('google_calendar_connected') === '1') return 'Google Calendarを接続しました。家族予定を書き込むカレンダーを選んでください。';
...
export function CalendarIntegrationSettings({ returnTo = '/settings' }: { returnTo?: string }) {
  ...
  const reauthNeeded=rows.some((row)=>row.reauth_required);
  const callbackNotice=oauthCallbackNotice();
