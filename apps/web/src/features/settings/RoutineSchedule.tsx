...
function DropoffPickupMatrix({ householdId, members }: { householdId: string | null; members: ReturnType<typeof useHousehold>['members'] }) {
  ...
  const save = async (row: MatrixRow) => { if (!row.id) return; setError(null); setNotice(null); setSaving(`${row.code}-${row.weekday}`); try { if (!row.assigneeId) await callEdgeFunction(EDGE_FUNCTIONS.deactivateRecurrence, { operation_id: newOperationId(), task_definition_id: row.id, weekday: row.weekday }); else await callEdgeFunction(EDGE_FUNCTIONS.changeRecurrence, { operation_id: newOperationId(), task_definition_id: row.id, weekday: row.weekday, assignee_strategy: 'fixed', planned_assignee_user_id: row.assigneeId, scheduled_local_time: row.localTime || undefined, effective_from: new Date().toLocaleDateString('sv-SE') }); setNotice('これから毎回の担当を保存しました。'); await load(); } catch (err) { setError(err instanceof FamilyOpsApiError ? err.message : '保存に失敗しました。'); } finally { setSaving(null); } };
