...
type Row = { id: string; title: string; is_active: boolean; include_in_routine_line: boolean; weekdays: number[]; localTime: string; strategy: string; assignee: string; fallbackAssignee: string; subtasks: SubtaskDraft[] };
...
export function CustomRoutineEditor({ householdId, members, kind }: { householdId: string | null; members: HouseholdMemberWithProfile[]; kind: 'morning_chore' | 'evening_chore' }) {
  ...
  const load = useCallback(async () => {
    ...
    const [defs, rules, subtasks] = await Promise.all([
      ...
      supabase.from('recurrence_rules').select('task_definition_id,weekday,scheduled_local_time,assignee_strategy,planned_assignee_id,fallback_assignee_id').eq('household_id', householdId).eq('active', true),
    ...
    setRows((defs.data ?? []).filter((definition) => isCustomRoutineDefinition(kind, definition.code)).map((definition) => {
      ...
      return { id: definition.id, title: definition.title, is_active: definition.is_active, include_in_routine_line: definition.include_in_routine_line,
        ...
        strategy: definitionRules[0]?.assignee_strategy ?? 'fixed', assignee: definitionRules[0]?.planned_assignee_id ?? '', fallbackAssignee: definitionRules[0]?.fallback_assignee_id ?? '',
  ...
  const add = async () => {
    ...
    try {
      ...
      await save({ id: created.task_definition_id, title: title.trim(), is_active: true, include_in_routine_line: line, weekdays: days, localTime: time, strategy, assignee, fallbackAssignee, subtasks });
