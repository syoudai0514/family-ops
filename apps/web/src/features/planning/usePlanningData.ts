...
export function usePlanningData(householdId: string | null, start: string, end: string) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [{ data: taskRows, error: taskError }, { data: calendarRows, error: calendarError }] =
        await Promise.all([
          supabase
            .from('task_instances')
            .select('*')
          ...
          supabase
            ...
            .lt(
              'starts_at',
              `${tokyoIsoDate(addDays(new Date(`${end}T00:00:00+09:00`), 1))}T00:00:00+09:00`,
            )
            .order('starts_at'),
        ]);
      if (taskError) throw taskError;
      // Calendar is optional (for households that have not connected Google).
      if (calendarError && !/does not exist|permission/i.test(calendarError.message))
        throw calendarError;
      setTasks(taskRows ?? []);
      setEvents(
        (calendarRows ?? []).map(
          (row: { occurrence_key: string; starts_at: string; title: string | null }) => ({
            id: row.occurrence_key,
            date: tokyoIsoDate(row.starts_at),
            time: row.starts_at,
            title: row.title || '予定',
            kind: 'calendar',
            assigneeId: null,
    ...
    () =>
      [
        ...tasks.map((task) => ({
          ...
          title: task.title,
          kind: 'task' as const,
          assigneeId: task.planned_assignee_id,
        })),
        ...events,
