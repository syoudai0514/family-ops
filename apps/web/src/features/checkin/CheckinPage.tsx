...
export function checkinBulkScopeNames(items: Array<{ title: string; expectation?: TaskExpectation }>): { eligible: string[]; optional: string[] } {
  const eligible: string[] = [];
  const optional: string[] = [];
  for (const item of items) {
    if ((item.expectation ?? 'normal') === 'optional') optional.push(item.title);
    else eligible.push(item.title);
  }
...
export function CheckinPage() {
  ...
  const scopeNames = checkinBulkScopeNames(activeItems);
  ...
  return <div className="app-shell">
    ...
    {canCorrectBulk && <section className="card checkin-reconciliation" aria-label="一括完了の直後操作">
      <h2>一括完了を記録しました</h2><p className="empty-hint">この操作で変更した項目だけを訂正できます。後から別の更新が入った場合、元に戻す操作は安全のため止まります。</p>
      <div className="checkin-reconciliation-actions"><button type="button" className="secondary-button" disabled={busyAll} onClick={() => setIndividualMode(true)}>例外を修正</button><button type="button" className="text-button" disabled={busyAll} onClick={undoReconciliation}>元に戻す</button></div>
    </section>}
    ...
    <ul className="handover-list">
      {session.items.map((item) => {
        ...
        return <li key={item.task_instance_id} className="handover-item">
          ...
          {canEditItem && <div className="checkin-item-actions"><button type="button" className="hero-primary" disabled={busyItemId === item.task_instance_id || busyAll} onClick={() => runItemAction(item.task_instance_id, 'complete')}>完了</button><details className="task-overflow"><summary aria-label={`${item.title}のその他の結果`}>その他の結果</summary><div><button type="button" disabled={busyItemId === item.task_instance_id || busyAll} onClick={() => runItemAction(item.task_instance_id, 'partner_handled')}>相手が対応</button><button type="button" disabled={busyItemId === item.task_instance_id || busyAll} onClick={() => runItemAction(item.task_instance_id, 'failed')}>できなかった</button><button type="button" disabled={busyItemId === item.task_instance_id || busyAll} onClick={() => runItemAction(item.task_instance_id, 'skip')}>今回は不要</button><button type="button" disabled={busyItemId === item.task_instance_id || busyAll} onClick={() => runItemAction(item.task_instance_id, 'cancelled')}>中止</button><label>再予定日<input type="date" aria-label={`${item.title}の再予定日`} value={rescheduleDates[item.task_instance_id] ?? ''} onChange={(event) => setRescheduleDates((current) => ({ ...current, [item.task_instance_id]: event.target.value }))} /></label><button type="button" disabled={busyItemId === item.task_instance_id || busyAll || !rescheduleDates[item.task_instance_id]} onClick={() => runItemAction(item.task_instance_id, 'rescheduled')}>再予定</button><button type="button" disabled={busyItemId === item.task_instance_id || busyAll} onClick={() => runItemAction(item.task_instance_id, 'unknown')}>不明</button></div></details></div>}
        </li>;
      })}
