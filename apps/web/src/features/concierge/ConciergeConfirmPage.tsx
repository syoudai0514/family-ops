...
export function ConciergeConfirmPage() {
  ...
  const hasActual = candidates.some((candidate) => candidate.kind === 'actual');
  const [busy, setBusy] = useState(false);
  const [results, setResults] = useState<ConciergeCommitResult[] | null>(null);

  async function register() {
    if (busy || unresolved.length > 0 || candidates.length === 0 || hasActual) return;
    setBusy(true);
  ...
  return <div className="app-shell concierge-page">
    ...
    {hasActual && <section className="card"><b>実績の登録経路を分離しています</b><p>予定外実績は、対象日と監査時刻を混同しない原子的な専用登録経路からのみ確定します。</p><button type="button" className="secondary-button" onClick={() => navigate('/actuals/new', { state })}>予定外実績へ戻る</button></section>}
    ...
    {!results && <button type="button" className="concierge-wide" disabled={busy || unresolved.length > 0 || candidates.length === 0 || hasActual} onClick={() => void register()}>{busy ? '登録中…' : '登録する'}</button>}
    ...
    {!allSaved && !results && <p className="meta">確認後の登録は既存の canonical Edge command を使います。途中で失敗した候補は成功扱いにしません。</p>}
  </div>;
}
