...
--   1) school_context_candidate rejects arbitrary/non-household identities;
...
--      reaches the ordinary command, while durable facts/candidates stay a
--      DB-issued constant quarantine representation independent of the secret.
--
-- Important: the command contract accepts required_item.normalized_value.item;
-- item_code is the post-minimization representation.  This attack therefore
-- uses only values accepted by the actual validator.
...
declare
  v_k integer:=least(p_k,p_n-p_k); v_i integer; v_result numeric:=1;
begin
...
declare
  v_n integer:=p_symbol_count+p_k-1; v_rank numeric:=p_rank;
  v_result integer[]:='{}'::integer[]; v_prev integer:=-1;
  v_i integer; v_y integer; v_count numeric; v_chosen boolean;
begin
...
declare
  v_k integer:=cardinality(p_values); v_n integer:=p_symbol_count+cardinality(p_values)-1;
  v_rank numeric:=0; v_prev integer:=-1; v_i integer; v_y integer; v_target integer;
begin
...
returns numeric language plpgsql immutable as $$
declare v_result numeric:=0; v_i integer;
begin
...
declare
  v_value numeric:=trunc(p_value); v_result bytea:=decode(repeat('00',p_length),'hex');
  v_i integer; v_byte integer;
begin
...
declare
  ...
  v_doc uuid; v_extraction uuid; v_extraction2 uuid;
  ...
  v_fact_id uuid; v_candidate_id uuid; v_count integer; v_error text;
  v_candidate_updated_at timestamptz; v_extraction_updated_at timestamptz;
begin
  ...
  ) values(v_household,v_child,'School A','A','2030-01-01',array['A'])
  returning id into v_context_a;
  insert into public.child_school_contexts(
    household_id,child_id,school_display_name,class_display_name,effective_from,recognition_aliases
  ) values(v_household,v_child,'School B','B','2030-01-01',array['B'])
  ...
  -- HIGH 2: hostile caller identity reaches ordinary intake but not durable key.
  v_intake:=private.fn_command_create_nursery_intake_v1(
    v_household,v_owner,v_actor,null,
    '58000000-0000-0000-0000-000000000101','codmon_notice',
  ...
  ) then raise exception 'FAIL DD9 quarantine: caller storage key survived intake'; end if;

  begin
    update private.source_documents
    set storage_object_key='private/dd9/REINJECT/YAMADA/09077778888' where id=v_doc;
  ...
  ) values(v_household,v_actor,'codmon_notice','YAMADA|09077778888|direct-insert',now(),null)
  ...
  -- HIGH 1: arbitrary/nonexistent/foreign ids and simultaneous scope swaps fail.
  begin
  ...
  begin
    update private.document_extractions
    set household_id=v_other_household,source_document_id=v_direct_doc,
  ...
  -- HIGH 3: 693 ACTUAL LEGAL symbols = 11 recognized item strings x
  -- 21 quantity states (absent or 1..20) x 3 confidence bands.
  ...
  v_secret:=convert_to('YAMADA|09077778888|x@y.example|ABCD1234','UTF8');
  ...
  if cardinality(v_symbols)<>64 or pg_temp.dd9_rank_multiset(v_symbols,693)<>v_rank then
    raise exception 'FAIL DD9 quarantine: multiset rank/unrank not reversible';
  end if;
  v_roundtrip:=pg_temp.dd9_numeric_to_bytes(pg_temp.dd9_rank_multiset(v_symbols,693),39);
  if v_roundtrip<>v_secret then
    raise exception 'FAIL DD9 quarantine: secret not reconstructed byte-for-byte';
  end if;
  ...
  foreach v_symbol in array v_symbols loop
    ...
    v_normalized:=jsonb_build_object('item',v_items[v_item_idx+1]);
    ...
    v_facts:=v_facts||jsonb_build_array(jsonb_build_object(
      'child_school_context_id',v_context_b,'fact_kind','required_item',
      'normalized_value',v_normalized,'confidence_band',v_confidence
    ));
  end loop;

  select updated_at into v_extraction_updated_at
  from private.document_extractions where id=v_extraction;
  v_review:=private.fn_command_record_nursery_extraction_v1(
    v_household,v_owner,v_actor,null,
    '58000000-0000-0000-0000-000000000102',v_extraction,1,
    jsonb_build_object('child_school_context_id',v_context_b),v_facts,'[]'::jsonb,'pwa'
  );
  if v_review->>'state'<>'review' or (v_review->>'source_fact_count')::integer<>64
    ...
    raise exception 'FAIL DD9 quarantine: legal hostile multiset did not reach review: %',v_review;
  end if;
  if (select updated_at from private.document_extractions where id=v_extraction)
       is distinct from v_extraction_updated_at then
    raise exception 'FAIL DD9 quarantine: extraction update timestamp became durable channel';
  end if;
  ...
  select count(*) into v_count from private.document_facts where extraction_id=v_extraction;
  if v_count<>64 then raise exception 'FAIL DD9 quarantine: expected 64 fact slots, got %',v_count; end if;
  if exists(
    select 1 from private.document_facts f
    where f.extraction_id=v_extraction and (
      ...
      or f.confidence_band<>'low' or f.source_locator is not null
    )
  ) then raise exception 'FAIL DD9 quarantine: model-controlled fact tuple survived'; end if;

  select count(*) into v_count from public.change_candidates c
  where c.source_type='ai_inference' and c.source_ref=v_extraction::text;
  if v_count<>32 then raise exception 'FAIL DD9 quarantine: expected 32 candidate slots, got %',v_count; end if;

  -- A second, different 39-byte legal multiset must create identical durable
  -- semantics.  This is the direct proof that rank cannot be reconstructed.
  v_secret2:=convert_to('SUZUKI|08099998888|z@q.example|WXYZ5678','UTF8');
  ...
  if pg_temp.dd9_numeric_to_bytes(pg_temp.dd9_rank_multiset(v_symbols2,693),39)<>v_secret2 then
    raise exception 'FAIL DD9 quarantine: second multiset not reversible pre-persistence';
  end if;
  ...
  -- Direct service-role writes cannot alter tuple state, slot count, or metadata.
  ...
  set fact_kind='required_item',normalized_value=jsonb_build_object('item','wipes','quantity',20),
      confidence_band='high',source_locator='item:65535'
  where id=v_fact_id;
  if not exists(
    select 1 from private.document_facts f where f.id=v_fact_id
      and f.fact_kind='event' and f.confidence_band='low' and f.source_locator is null
      and f.normalized_value->>'value_kind'='r0_untrusted_source_fact_withheld'
  ...
  exception when others then
    v_error:=sqlerrm; if v_error like 'FAIL DD9 quarantine:%' then raise; end if;
  end;
  begin
  ...
  select id,updated_at into v_candidate_id,v_candidate_updated_at
  ...
  if not exists(
    select 1 from public.change_candidates c where c.id=v_candidate_id
      and c.target_type='info' and c.target_id is null and c.current_snapshot_hash is null
      and c.status='pending' and c.revision=1
      and c.proposed_patch->>'reason_code'='model_candidate_requires_review'
      and c.proposed_patch->>'value_kind'='r0_untrusted_ai_candidate_withheld'
      and c.updated_at=v_candidate_updated_at
  ) then raise exception 'FAIL DD9 quarantine: service-role candidate UPDATE changed fixed slot/metadata'; end if;
