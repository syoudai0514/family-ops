...
declare
  ...
  household jsonb; household_id uuid; dinner_id uuid; before_rules jsonb; after_rules jsonb;
  rejected boolean := false;
begin
  household := public.server_tx_create_household(owner_id, '33000000-0000-0000-0000-000000000011', 'Custom boundary', 'Owner');
  household_id := (household->>'household_id')::uuid;
  select id into dinner_id from public.task_definitions where household_id=household_id and code='dinner';
  ...
  values
    (household_id,dinner_id,1,'builtin-monday','fixed','18:00','2026-09-01',true,owner_id),
    (household_id,dinner_id,5,'builtin-friday','pickup_assignee','20:30','2026-09-01',true,owner_id);
  ...
  into before_rules from public.recurrence_rules where household_id=household_id and task_definition_id=dinner_id and active;
  begin
  ...
  select jsonb_agg(jsonb_build_object('weekday',weekday,'strategy',assignee_strategy,'time',scheduled_local_time) order by weekday)
  into after_rules from public.recurrence_rules where household_id=household_id and task_definition_id=dinner_id and active;
  ...
  if (select count(*) from public.task_definitions where household_id=household_id and code in ('dinner','bath','laundry','dishes','cleaning','smile_zemi','media_30min')) <> 7 then
