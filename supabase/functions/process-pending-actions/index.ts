...
type DraftRow = Pick<PendingActionItem, 'id' | 'household_id' | 'actor_id' | 'action_type' | 'normalized_payload'>;
...
async function processLineDrafts(client: SupabaseClient): Promise<{ previewed: number; ambiguous: number; failed: number }> {
  ...
  for (const row of rows) {
    try {
      ...
      if (!prepared) {
        ...
        const { error: markError } = await client.rpc('server_tx_mark_line_pending_parse_failed', { p_id: row.id });
      ...
      const { error: markError } = await client.rpc('server_tx_mark_line_pending_previewed', { p_id: row.id });
      if (markError) throw new Error(markError.message);
      previewed++;
    } catch (err) {
      failed++;
  ...
  switch (item.action_type) {
    case 'shopping_item_add': {
      const { data, error } = await client.rpc('server_tx_add_shopping_item', {
        p_actor_id: item.actor_id, p_operation_id: item.operation_id,
        p_title: String(p.title ?? ''), p_purchase_method: String(p.purchase_method ?? 'store'),
        p_assignee_user_id: p.assignee_user_id ?? null, p_url: p.url ?? null, p_due_at: p.due_at ?? null,
      });
      if (error) throw new Error(error.message);
    ...
    case 'task_create_once': {
      const { data, error } = await client.rpc('server_tx_create_task', {
        p_actor_id: item.actor_id, p_operation_id: item.operation_id,
        p_title: String(p.title ?? ''), p_category: p.category ?? 'todo',
        p_scheduled_date: p.scheduled_date, p_due_local_time: p.due_local_time ?? null,
        p_planned_assignee_user_id: p.planned_assignee_user_id ?? null,
        p_completion_mode: 'whole', p_routine_phase: p.routine_phase ?? 'anytime', p_subtasks: null,
      });
      if (error) throw new Error(error.message);
    ...
    case 'request_create': {
      ...
      const { data, error } = await client.rpc('server_tx_send_request', {
        p_actor_id: item.actor_id, p_operation_id: item.operation_id,
        p_recipient_user_id: recipient, p_shared_title: String(p.title ?? ''),
        p_shared_message: typeof p.shared_message === 'string' ? p.shared_message : null,
        p_due_at: requestDueAt(p),
      });
      if (error) throw new Error(error.message);
      return { result_type: 'request', result_id: (data as { request_id?: string })?.request_id ?? null };
    }
    case 'request_accept': {
      ...
      const { data, error } = await client.rpc('server_tx_accept_request', {
        p_actor_id: item.actor_id, p_operation_id: item.operation_id, p_request_id: requestId,
      });
      if (error) {
        if (/REQUEST_(?:NOT_PENDING|ACCEPT_NOT_ALLOWED)/.test(error.message)) return { result_type: 'request', result_id: requestId };
    ...
    case 'request_decline': {
      ...
      if (error) {
        if (/REQUEST_(?:NOT_PENDING|DECLINE_NOT_ALLOWED)/.test(error.message)) return { result_type: 'request', result_id: requestId };
        throw new Error(error.message);
      }
      return { result_type: 'request', result_id: (data as { request_id?: string })?.request_id ?? requestId };
    }
    case 'assignment_change_request': {
      const { data, error } = await client.rpc('server_tx_create_assignment_change_request', {
        p_actor_id: item.actor_id, p_operation_id: item.operation_id, p_task_id: p.task_id,
        p_recipient_user_id: p.recipient_user_id, p_shared_message: p.shared_message ?? null,
...
Deno.serve(
  withServiceHandler(async (req: Request) => {
    ...
    const { data: batchData, error: claimError } = await client.rpc('server_tx_claim_pending_actions_batch', {
      p_worker_id: WORKER_ID, p_limit: BATCH_LIMIT, p_lease_seconds: LEASE_SECONDS,
    });
    ...
    for (const item of items) {
      try {
        ...
        const { data: completeData } = await client.rpc('server_tx_complete_pending_action', {
          ...
          p_result_type: outcome.result_type, p_result_id: outcome.result_id,
        });
        ...
        await client.rpc('server_tx_fail_pending_action', {
          p_id: item.id, p_lease_token: item.lease_token, p_error: message.slice(0, 500),
          p_max_attempts: MAX_ATTEMPTS, p_retry_delay_seconds: RETRY_DELAY_SECONDS,
        });
      }
    }
