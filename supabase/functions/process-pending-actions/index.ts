...
const LEASE_SECONDS = Number(
  Deno.env.get("PENDING_ACTIONS_LEASE_SECONDS") ?? "55",
);
const MAX_ATTEMPTS = Number(
  Deno.env.get("PENDING_ACTIONS_MAX_ATTEMPTS") ?? "5",
);
const RETRY_DELAY_SECONDS = Number(
  Deno.env.get("PENDING_ACTIONS_RETRY_DELAY_SECONDS") ?? "30",
);
const LINE_DRAFT_BATCH_LIMIT = Number(
  Deno.env.get("LINE_DRAFT_BATCH_LIMIT") ?? "20",
);
...
function scheduleLabel(payload: Record<string, unknown>): string {
  ...
  const time = typeof payload.due_local_time === "string"
...
async function prepareDraft(
  ...
  if (row.action_type === "task_create_once") {
    return {
      ...
      payload: {
        ...original,
        planned_assignee_user_id: original.planned_assignee_user_id ??
          row.actor_id,
        category: original.category ?? "todo",
        routine_phase: original.routine_phase ?? "anytime",
        calendar_visibility: original.calendar_visibility === "special"
          ? "special"
          : "hidden",
        target_label: "自分",
      },
  ...
  if (row.action_type === "shopping_item_add") {
    return {
      ...
      payload: {
        ...original,
        target_label: original.target_label ?? "買い物リスト",
      },
      kindLabel: "買い物",
      targetLabel: "買い物リスト",
    };
  ...
  const rawText = typeof original.raw_text === "string"
    ? original.raw_text.trim()
    : "";
  ...
  const requestedRecipient = intent.kind === "request"
    ? (roleUser ??
...
async function enqueueDraftPreview(
  ...
  const { error } = await client.rpc("server_tx_enqueue_immediate_line_push", {
    ...
    p_rich_message: buildPendingActionPreviewFlex({
      ...
      confirmLabel: prepared.actionType === "request_create"
        ? "この内容で送る"
        : "この内容で登録",
    }),
  });
    ...
    try {
      ...
      if (!prepared) {
        ...
        const { error: markError } = await client.rpc(
          "server_tx_mark_line_pending_parse_failed",
          {
            p_id: row.id,
          },
        );
      ...
      const { error: markError } = await client.rpc(
        "server_tx_mark_line_pending_previewed",
        {
          p_id: row.id,
        },
      );
...
function requestDueAt(payload: Record<string, unknown>): string | null {
  ...
  const localTime = typeof payload.due_local_time === "string" &&
      /^\d{2}:\d{2}$/.test(payload.due_local_time)
    ? payload.due_local_time
    : "23:59";
  ...
  switch (item.action_type) {
    case "shopping_item_add": {
      ...
      return {
        ...
        result_id: (data as { shopping_item_id?: string })?.shopping_item_id ??
          null,
      };
    }
    case "task_create_once": {
      ...
      const { data, error } = await client.rpc(
        "server_tx_create_task_with_calendar",
        {
          ...
          p_calendar_visibility: p.calendar_visibility === "special"
            ? "special"
            : "hidden",
    ...
    case "request_create": {
      const recipient = typeof p.recipient_user_id === "string"
        ? p.recipient_user_id
        : "";
      if (!recipient) {
        throw new Error("request_create missing recipient_user_id");
      }
      const { data, error } = await client.rpc("server_tx_send_request", {
        ...
        p_shared_message: typeof p.shared_message === "string"
          ? p.shared_message
          : null,
    ...
    case "line_multi_intent_review": {
      ...
      if (candidates.length === 0) {
        throw new Error("multi-intent review has no candidates");
      }
      ...
      for (const candidate of candidates) {
        ...
        const decision = (candidate as typeof candidate & {
          duplicate_decision?: "existing" | "update" | "separate" | null;
        }).duplicate_decision ?? null;
        if (duplicate && !decision) {
          throw new Error("multi-intent duplicate still needs a decision");
        }
        ...
        if (duplicate && decision === "update") {
          const { error } = await client.rpc(
            "server_tx_commit_concierge_duplicate_update",
            {
              ...
              p_planned_assignee_user_id:
                candidate.action_type === "task_create_once"
                  ? candidate.payload.planned_assignee_user_id ?? null
                  : null,
            },
          );
          if (error) throw new Error(error.message);
    ...
    for (const item of items) {
      try {
        ...
        const { data: completeData } = await client.rpc(
          "server_tx_complete_pending_action",
          {
            p_id: item.id,
        ...
        console.error("process-pending-actions: execution failed", {
          id: item.id,
          message,
        });
