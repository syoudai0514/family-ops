...
Deno.serve(withServiceHandler(async (req: Request) => {
  ...
  for (const item of claimed) {
    try {
      ...
      const { data: prepared, error: prepareError } = await client.rpc('server_tx_prepare_nursery_line_source',{p_intake_id:item.id,p_expected_revision:item.revision,p_storage_object_key:objectKey,p_parser_version:'nursery-line-v1'});
      if (prepareError) throw prepareError;
      const preparedRevision = Number(prepared.revision);
