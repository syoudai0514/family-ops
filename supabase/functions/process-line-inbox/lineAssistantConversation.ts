...
export function assistantConversationFallback(text: string): string {
  ...
  if (
    /(?:送らないで|送らず|おくらず|登録せず|登録しない|とうろくなし|相談だけ|そうだんだけ|文章だけ考えて|ぶんしょうだけかんがえて)/u
      .test(value)
  ) {
    return "了解。これは相談として扱い、家族への送信や登録はしません。伝えたい内容があれば、そのまま書いてください。";
  }
  if (/(?:どう言えば|なんて言えば|どう頼めば|どんな言い方|角立たない)/u.test(value)) {
...
async function geminiConversationProvider(text: string): Promise<string | null> {
  ...
  const prompt = [
    ...
    "ただし『送信した』『登録した』『依頼した』など、実行していないmutationを行ったように言ってはいけません。",
    "ユーザーが『送らない』『登録しない』『相談だけ』『文章だけ』と指定した場合は、その制約を短く確認してください。",
...
export function ambiguousAddresseeReply(): string {
  return "これは私への相談ですか？それとも家族へのお願いですか？ 相手にはまだ送らず、登録もしていません。";
}
