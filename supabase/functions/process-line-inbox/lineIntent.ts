...
export function deterministicLineIntent(text: string, now = new Date()): LineIntent | null {
  ...
  const requestSignal = /(?:してほしい|して欲しい|お願い|頼め|頼み|やってほしい|やって欲しい|してくれる|してもら|お願いでき)/.test(
    normalized,
  );
  ...
  if (insurance) {
    let prefix = cleanNoun(insurance[1] ?? '').replace(/(?:しなくちゃ.*|しないと.*)$/u, '').trim();
...
async function geminiLineIntent(text: string, now: Date): Promise<LineIntent | null> {
  ...
  try {
    ...
    return {
      ...
      daypart,
      targetRole: role,
      sharedMessage: typeof p.shared_message === 'string' && p.shared_message.trim()
...
export async function extractLineIntent(text: string, now = new Date()): Promise<LineIntent | null> {
