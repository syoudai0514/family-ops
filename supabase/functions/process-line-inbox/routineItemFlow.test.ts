...
Deno.test("buildItemQuickReply produces exactly the four required actions with no bearer secret", () => {
  ...
  assertEquals(actions.map((a) => a.label), [
    "完了",
    "相手が対応",
    "今回は不要",
    "次へ",
  ...
  for (const action of actions) {
    ...
    assert(
      action.data.includes("session_id=session-1"),
      "postback data must reference the session id",
    );
    ...
    assert(
      !/token|secret|bearer/i.test(action.data),
      `postback data must not reference a secret: ${action.data}`,
    );
  }
  assertEquals(
    actions[0].data,
    "action=routine_item&session_id=session-1&task_instance_id=task-1&value=complete",
  );
  assertEquals(
    actions[1].data,
    "action=routine_item&session_id=session-1&task_instance_id=task-1&value=partner_handled",
  );
  assertEquals(
    actions[2].data,
    "action=routine_item&session_id=session-1&task_instance_id=task-1&value=skip",
  );
  assertEquals(
    actions[3].data,
    "action=routine_item_next&session_id=session-1&task_instance_id=task-1",
  );
});

Deno.test("buildItemPromptText includes the item title and a PWA link (fallback path requirement #7)", () => {
  const text = buildItemPromptText("session-1", {
    task_instance_id: "b",
    title: "英語用品",
    status: "todo",
  });
  ...
  assert(
    text.includes("/checkin/session-1"),
    "prompt text must include the PWA checkin deep link",
  );
});

Deno.test("buildStaleSessionText links to the current session when known, else the stale one", () => {
  assert(
    buildStaleSessionText("current-id", "stale-id").includes(
      "/checkin/current-id",
    ),
