...
function hasConversationAdviceCue(value: string): boolean {
  return /(?:どう思う|どうおもう|どうかな|どうするのが(?:いい|良い|よい)(?:と思う|とおもう)?|どう(?:言|い)えば|どう(?:言|い)うのが(?:いい|良い|よい|自然)|なんて(?:言|い)えば|どう頼めば|どうたのめば|どう頼む|どうたのむ|どう(?:頼|たの)むのが(?:いい|良い|よい|自然)|どう伝える|どうつたえる|どんな言い方|どんないいかた|どういう言い方|どういういいかた|どんな感じ.*(?:角立たない|角が立たない)|(?:角立たない|角が立たない).*(?:言い方|いいかた|文章|文面)|お願いできる(?:と)?思う|お願いできる(?:と)?おもう|おねがいできる(?:と)?おもう|お願いできそう(?:かな|か)?|おねがいできそう(?:かな|か)?|お願いできるかな|おねがいできるかな|頼めそう(?:かな|か)?|たのめそう(?:かな|か)?|頼むなら.*(?:言い方|いいかた)|たのむなら.*(?:言い方|いいかた))/u
    .test(value);
}
...
function hasAssistantAddressCue(value: string): boolean {
  return /(?:^|[、,])(?:あなた|おうちノート|AI|エーアイ|そっち)(?:に|へ)?(?:相談|そうだん|聞|き|言|い)/u
    ...
    /(?:妻|ママ|まま|パパ|ぱぱ)(?:じゃなくて|ではなくて)(?:AI|エーアイ|あなた|おうちノート)(?:に|へ)?(?:聞|き|相談|そうだん)/u
      .test(roleSafeValue(value)) ||
    /(?:お願い|おねがい|依頼|いらい|送る|おくる|登録|とうろく)(?:じゃない|ではない|じゃなくて|ではなくて)[、,]?(?:あなた|AI|エーアイ|おうちノート)(?:に|へ)?(?:相談|そうだん|聞|き|意見)/u
      .test(value) ||
    /(?:家族|相手|パートナー)(?:に|へ)?(?:送る|おくる|伝える|つたえる)(?:ん|の)?(?:じゃなくて|ではなくて|ではなく)[、,]?(?:AI|エーアイ|あなた|おうちノート)(?:に|へ)?(?:相談|そうだん|聞|き)/u
      .test(value);
}
...
function hasDraftReviewCue(value: string): boolean {
  ...
  return new RegExp(
    `^${prefix}(?:この|その|あの)?(?:文章|文面|メッセージ|文案)(?:を)?(?:見て|みて|直して|なおして|整えて|ととのえて|チェックして|見てもらえる|みてもらえる)$`,
    "u",
  ).test(value) ||
    new RegExp(
      `^${prefix}(?:この|その|あの)?(?:言い方|いいかた)(?:を|だけ)?(?:見て|みて|直して|なおして|整えて|ととのえて|考えて|かんがえて|チェックして|見てもらえる|みてもらえる)$`,
      "u",
    ).test(value) ||
...
function clauseDisposition(text: string): ClauseDisposition {
  ...
  if (
    /^(?:これ|それ|そっち|こっち)(?:って)?(?:お願いできる|おねがいできる|頼める|たのめる|いける|任せていい|まかせていい|どう|どうする)$/u.test(value) ||
...
export function linePendingFollowUpKind(text: string): LinePendingFollowUpKind {
  ...
  if (/(?:だけやめて|取り消|キャンセル|やっぱ(?:り)?(?:さっきの)?なし|さっきのなし|やっぱなし|やっぱ(?:り)?今の(?:お願い|おねがい)?なし(?:で)?)/u.test(value)) {
    return "cancel";
  }
...
export function isAssistantAddressCorrection(text: string): boolean {
  ...
  return candidates.some((candidate) => {
    if (
      /^(?:(?:あなた|おうちノート|AI|そっち)(?:に|へ)?)(?:聞いてる|聞いている|聞いてんの|きいてる|相談(?:してる|している|中)?|そうだん(?:してる|している|中)?|言ってる|言っている|言ってんの)(?:んだけど|んだよ|んだけどね|よ)?$/u
        .test(candidate)
    ) return true;

    if (
      /^(?:妻|ママ|まま|パパ|ぱぱ)(?:じゃなくて|ではなくて)(?:AI|あなた|おうちノート)(?:に|へ)?(?:聞いてる|聞いている|きいてる|相談(?:してる|している|中)?|そうだん(?:してる|している|中)?)(?:んだけど|んだよ|よ)?$/u
        .test(roleSafeValue(candidate))
    ) return true;

    if (
      /^(?:お願い|おねがい|依頼|いらい|送って|おくって|送る|おくる|登録して|とうろくして|登録|とうろく)(?:じゃない|ではない|じゃなくて|ではなくて)[、,]?(?:あなた|AI|おうちノート)(?:に|へ)?(?:相談(?:してる|している|中)?|そうだん(?:してる|している|中)?|質問|しつもん|聞いてる|きいてる|意見(?:が)?(?:聞きたい|ききたい))?$/u
        .test(candidate)
    ) return true;

    if (
      /^(?:家族|相手|パートナー)(?:に|へ)?(?:送る|おくる|伝える|つたえる)(?:ん|の)?(?:じゃなくて|ではなくて|ではなく)[、,]?(?:AI|エーアイ|あなた|おうちノート)(?:に|へ)?(?:相談(?:してる|している|中)?|そうだん(?:してる|している|中)?|聞いてる|きいてる)?$/u
        .test(candidate)
    ) return true;
