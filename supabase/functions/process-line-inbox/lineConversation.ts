...
export function lineConversationalReplacementTitle(text: string): string | null {
  ...
  if (/^(?:自分|パパ|ぱぱ|ママ|まま|父|母|お父さん|お母さん|嫁さん|奥さん|妻)$/u.test(replacement)) return null;
  if (/^(?:自分|パパ|ぱぱ|ママ|まま|父|母|お父さん|お母さん|嫁さん|奥さん|妻)\s*(?:じゃなくて|ではなくて|じゃなく|ではなく|の代わりに)\s*(?:自分|パパ|ぱぱ|ママ|まま|父|母|お父さん|お母さん|嫁さん|奥さん|妻)$/u.test(replacement)) return null;
  return replacement;
}
