...
export function lineConversationalReplacementTitle(text: string): string | null {
  ...
  const roleToken = "(?:自分|パパ|ぱぱ|ママ|まま|父|母|お父さん|お母さん|嫁さん|奥さん|妻)";
  if (new RegExp(`^${roleToken}import type { LineQuickReplyAction } from "../_shared/lineMessaging.ts";
...
export function lineConversationalReplacementTitle(text: string): string | null {
  ...
  if (!match) return null;
  const replacement = match[1].replace(/\s+/g, " ").trim();
, "u").test(replacement)) return null;
  if (new RegExp(`^${roleToken}(?:じゃなくて|ではなくて|じゃなく|ではなく|の代わりに)${roleToken}import type { LineQuickReplyAction } from "../_shared/lineMessaging.ts";
