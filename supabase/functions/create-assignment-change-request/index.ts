...
Deno.serve(withUserMutationHandler(async (req: Request) => {
  ...
  const expectedRevision = typeof body.expected_task_revision === 'number' && Number.isInteger(body.expected_task_revision)
    ? body.expected_task_revision
    : null;
  const rpc = expectedRevision === null
  ...
  if (expectedRevision !== null) args.p_expected_task_revision = expectedRevision;
  const result = await callServerTx(createServiceRoleClient(), rpc, args);
