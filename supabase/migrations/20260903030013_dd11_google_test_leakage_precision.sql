-- A test fixture may contain a provider-less hostile mirror solely to prove
-- that the projector chooses a local DELETE/no-payload path.  It has no
-- provider identity and cannot be a production Google mutation leak.  A test
-- Task mirror with an exact provider identity is the blocking condition.
...
language sql stable security definer set search_path='' as $$
  ...
  where t.test_context_id is not null and nullif(btrim(coalesce(m.provider_event_id,'')),'') is not null
  union all
