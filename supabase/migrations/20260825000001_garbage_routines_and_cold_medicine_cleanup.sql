...
begin
  ...
  loop
    ...
    -- The cold-medicine course is finished. Keep history, but stop future
    -- recurrence and remove only still-pending future/today instances.
    ...
    insert into public.task_definitions(
      ...
      include_in_routine_line
    ) values (
      v_household_id,
      'garbage_recyclables_mon',
      '缶・ペットボトル・段ボールのゴミ出し',
      'garbage',
      'morning',
      'whole',
    ...
    if not exists (
      select 1
       ...
       where household_id = v_household_id
         and task_definition_id = v_def_id
         and weekday = 1
      ...
      insert into public.recurrence_rules(
        ...
        effective_from, active, version, created_by
      ) values (
        v_household_id, v_def_id, 1, 'default',
        'dropoff_assignee', null, time '07:30',
        v_today, true, 1, v_actor_id
      );
    end if;
    ...
    for v_weekday in select unnest(array[2, 6])
    loop
      if not exists (
        select 1
          from public.recurrence_rules
        ...
        insert into public.recurrence_rules(
          ...
          v_household_id, v_def_id, v_weekday, 'default',
          'dropoff_assignee', null, time '07:30',
          v_today, true, 1, v_actor_id
        );
      end if;
    ...
    -- Wednesday: plastic packaging garbage.
    insert into public.task_definitions(
      household_id, code, title, category, routine_phase, completion_mode,
      is_active, sort_order, created_by, calendar_visibility, task_kind,
      include_in_routine_line
    ...
    if not exists (
      ...
      insert into public.recurrence_rules(
        ...
        v_household_id, v_def_id, 3, 'default',
        'dropoff_assignee', null, time '07:30',
        v_today, true, 1, v_actor_id
      );
    end if;
