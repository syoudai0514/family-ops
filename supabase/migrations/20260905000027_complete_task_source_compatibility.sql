-- Routine LINE/PWA automation calls the source-aware six-argument completion
-- API, while the public PWA Edge adapter still calls the historical five-
-- argument shape. Keep both exact signatures, with NO default on the six-arg
-- function, so a five-arg call (including an untyped NULL boolean) is never
-- ambiguous. The five-arg function delegates to the single source-aware
-- implementation with source='pwa'.
...
begin
  ...
  if v_source not in ('pwa','line') then raise exception 'INVALID_INPUT'; end if;
  ...
  insert into public.task_events(
    ...
    v_source,p_operation_id::text||':completed'
  );
...
set search_path = ''
as $$
  select public.server_tx_complete_task(
    p_actor_id,p_operation_id,p_task_id,p_completion_actor,p_complete_remaining_subtasks,'pwa'::text
  );
$$;

revoke all on function public.server_tx_complete_task(uuid,uuid,uuid,text,boolean,text) from public,anon,authenticated;
