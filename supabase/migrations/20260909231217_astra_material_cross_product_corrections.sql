-- Astra XC-01: partial duplicate updates preserve omitted owner/deadline/calendar
-- fields. Assignment changes require the existing agreement command; matching
-- a title is never consent to replace a household member. Receipts still replay
-- before CAS and an update conflict never falls through to create.
...
begin
  ...
  if p_entity_kind = 'task' then
    ...
    if p_planned_assignee_user_id is not null
       and p_planned_assignee_user_id is distinct from v_task.planned_assignee_id then
    ...
    perform public.server_tx_edit_task_with_calendar(
      ...
      p_due_local_time => coalesce(p_due_local_time, (v_task.due_at at time zone 'Asia/Tokyo')::time),
      p_calendar_end_local_time => (v_task.calendar_ends_at at time zone 'Asia/Tokyo')::time,
      p_category => null,
      p_planned_assignee_user_id => v_task.planned_assignee_id,
    ...
    set title = btrim(p_title),
        due_at = case
          ...
          else (p_scheduled_date + coalesce(p_due_local_time, (due_at at time zone 'Asia/Tokyo')::time, time '23:59')) at time zone 'Asia/Tokyo'
        end,
        revision = revision + 1
    where household_id = v_household_id and id = p_entity_id
