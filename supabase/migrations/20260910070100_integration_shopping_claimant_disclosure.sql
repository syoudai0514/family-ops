...
--
-- Keep Shopping ownership truth in shopping_items.active_claimant_actor_ref_id.
-- This read-model-only change adds the current real claimant's household profile
-- display name so the PWA can identify who is actually handling an anyone item.
-- No claim TTL/expiry semantics are introduced: release remains explicit.
