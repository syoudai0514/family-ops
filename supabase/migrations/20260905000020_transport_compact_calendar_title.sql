-- Issue #48 final UX contract: narrow transport presentation is exact.
-- Canonical assignments remain separate; only the Google/month title is
-- projected as 送P迎M / 送P / 迎M with no whitespace or separators.
...
as $$
  select
    case when p_dropoff_user_id is null then '' else '送' || coalesce(private.family_ops_member_token(p_household_id,p_dropoff_user_id),'') end
    || case when p_pickup_user_id is null then '' else '迎' || coalesce(private.family_ops_member_token(p_household_id,p_pickup_user_id),'') end;
$$;
