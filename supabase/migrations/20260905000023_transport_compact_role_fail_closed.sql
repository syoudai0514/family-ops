-- Final transport compact-title hardening. An assigned transport actor must
-- resolve to the explicit household P/M role before a narrow presentation is
-- emitted. Never fall back to a Japanese label, join order, blank token, or
-- guessed role because the final UX contract requires ASCII P/M abbreviations.
...
begin
  if p_dropoff_user_id is not null then
    ...
    if v_dropoff_token not in ('P','M') then
  ...
  if p_pickup_user_id is not null then
    ...
    if v_pickup_token not in ('P','M') then
      raise exception 'TRANSPORT_COMPACT_ACTOR_TOKEN_REQUIRED';
    end if;
